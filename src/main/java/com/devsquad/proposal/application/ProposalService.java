...
public class ProposalService {
  ...
  @Transactional
  public ProposalStore.ProposalView create(
      String clerkId, UUID hubId, ProposalStore.ProposalContent content) {
    var authorId = authorization.requireHubMember(clerkId, hubId);
    var proposal =
        ProjectProposal.draft(
            UUID.randomUUID(), hubId, authorId, content.title(), content.summary());
    return proposals.create(proposal.id(), hubId, authorId, normalize(content));
  }

  @Transactional
  public ProposalStore.ProposalView edit(
      String clerkId, UUID proposalId, ProposalStore.ProposalContent content) {
  ...
  public ProposalStore.ProposalView approve(String clerkId, UUID proposalId) {
    ...
    return proposals.approveAndCreate(
        proposal,
        content,
        slug(content.title()) + "-" + suffix,
        "PRJ" + suffix.toUpperCase(Locale.ROOT));
  }
  ...
  private void requireAuthor(String clerkId, ProjectProposal proposal) {
    if (!authorization.requireHubMember(clerkId, proposal.hubId()).equals(proposal.authorId())) {
      throw new DomainException(
          "proposal_author_required", "Only the proposal author can perform this operation");
    }
  }
  ...
  private static ProposalStore.ProposalContent normalize(ProposalStore.ProposalContent content) {
    return new ProposalStore.ProposalContent(
        ...
        trim(content.problem()),
        trim(content.proposedSolution()),
        trim(content.goals()),
        content.desiredSkills() == null ? List.of() : content.desiredSkills());
  }
  ...
  private static String slug(String value) {
    ...
    var slug =
        normalized.toLowerCase(Locale.ROOT).replaceAll("[^a-z0-9]+", "-").replaceAll("(^-|-$)", "");
