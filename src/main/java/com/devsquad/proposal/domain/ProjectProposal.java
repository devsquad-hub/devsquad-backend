...
public final class ProjectProposal {
  ...
  public static ProjectProposal draft(
      UUID id, UUID hubId, UUID authorId, String title, String summary) {
  ...
  public void edit(String title, String summary) {
    if (status != ProposalStatus.DRAFT && status != ProposalStatus.REJECTED) {
      throw new DomainException(
          "proposal_not_editable", "An approved or pending proposal cannot be edited");
    }
    this.title = requireText(title, "title");
    this.summary = requireText(summary, "summary");
  }
  ...
  public void submit() {
    if (status != ProposalStatus.DRAFT && status != ProposalStatus.REJECTED) {
      throw new DomainException(
          "proposal_not_submittable", "Proposal cannot be submitted from " + status);
    }
