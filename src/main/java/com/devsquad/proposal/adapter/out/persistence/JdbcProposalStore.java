...
public class JdbcProposalStore implements ProposalStore {
  ...
  public ProposalView create(UUID id, UUID hubId, UUID authorId, ProposalContent content) {
    jdbc.sql(
        ...
        .param("problem", content.problem())
        .param("solution", content.proposedSolution())
        .param("goals", content.goals())
        .param("skills", pgArray(content.desiredSkills()))
        .update();
    return view(id);
  }
  ...
  public ProposalView saveState(ProjectProposal proposal) {
    jdbc.sql(
            ...
            """)
        .param("status", proposal.status().name())
        .param("reviewerId", proposal.reviewerId())
        .param("reason", proposal.decisionReason())
        .param("projectId", proposal.projectId())
        .param("id", proposal.id())
        .update();
    return view(proposal.id());
  }
  ...
  @Override
  public ProposalView approveAndCreate(
    ...
    jdbc.sql(
            ...
            """)
        .param("id", proposal.projectId())
        .param("hubId", proposal.hubId())
        .param("proposalId", proposal.id())
        .param("name", content.title())
        .param("slug", projectSlug)
        .param("projectKey", projectKey)
        .param("summary", content.summary())
        .param("description", content.proposedSolution())
        .param("tags", pgArray(content.desiredSkills()))
        .update();
    jdbc.sql(
            """
            ...
            """)
        .param("projectId", proposal.projectId())
        .param("authorId", proposal.authorId())
        .update();
    ...
    jdbc.sql(
            ...
            """)
        .param("receiverId", proposal.authorId())
        .param("actorId", proposal.reviewerId())
        .param("hubId", proposal.hubId())
        .param("projectId", proposal.projectId())
        .param("proposalId", proposal.id())
        .update();
  ...
  public List<ProposalView> findByHub(UUID hubId, UUID actorId, boolean canReview) {
    var sql =
        VIEW_SELECT
            + " where p.hub_id = :hubId "
            + (canReview ? "" : "and p.author_id = :actorId ")
  ...
  private ProposalView view(UUID id) {
    return jdbc.sql(VIEW_SELECT + " where p.id = :id")
        .param("id", id)
        .query(JdbcProposalStore::mapView)
        .single();
  }
  ...
  private static ProposalContent content(ResultSet rs) throws SQLException {
    ...
    return new ProposalContent(
        ...
        rs.getString("proposed_solution"),
        rs.getString("goals"),
        List.copyOf(skills));
  }
