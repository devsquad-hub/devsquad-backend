...
public class JdbcRecruitmentStore implements RecruitmentStore {
    ...
    public UUID createRound(UUID projectId, RoundCommand command) {
        ...
        jdbc.sql("""
                        ...
                        """).param("id", id).param("projectId", projectId).param("name", command.name())
                .param("description", command.description()).param("opensAt", command.opensAt())
                .param("closesAt", command.closesAt()).update();
        return id;
    }

    @Override
    public UUID projectForRound(UUID roundId) {
        return jdbc.sql("select project_id from recruitment_rounds where id = :id").param("id", roundId)
                .query(UUID.class).optional()
                .orElseThrow(() -> new DomainException("recruitment_round_not_found", "Round was not found"));
    }
    ...
    public UUID createPosition(UUID roundId, PositionCommand command) {
        ...
        jdbc.sql("""
                        ...
                        """).param("id", positionId).param("roundId", roundId).param("title", command.title())
                .param("description", command.description()).param("skills", pgArray(command.skills()))
                .param("capacity", command.capacity()).update();
        jdbc.sql("insert into recruitment_form_versions (id, position_id, version) values (:id, :positionId, 1)")
                .param("id", formId).param("positionId", positionId).update();
        var order = 0;
        for (var question : command.questions()) {
            jdbc.sql("""
                            ...
                            """).param("formId", formId).param("key", question.key()).param("label", question.label())
                    .param("type", question.type()).param("required", question.required()).param("position", order++)
                    .param("options", json(question.options())).update();
        }
        return positionId;
    }
    ...
    public void openRound(UUID roundId, UUID projectId) {
        var opened = jdbc.sql("""
                        ...
                        """).param("id", roundId).param("project", projectId).update();
        if (opened == 0) {
            throw new DomainException("round_not_openable", "Round or project is not in an openable state");
        }
        jdbc.sql("update projects set status = 'RECRUITING', updated_at = now() where id = :id and status = 'PLANNING'")
    ...
    public List<PositionView> publicPositions(UUID projectId) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new PositionView(rs.getObject("id", UUID.class),
                        rs.getObject("round_id", UUID.class), rs.getString("round_name"), rs.getString("title"),
                        rs.getString("description"), List.of((String[]) rs.getArray("skills").getArray()),
                        rs.getInt("capacity"), rs.getInt("filled"), rs.getObject("closes_at", OffsetDateTime.class),
    ...
    private List<QuestionView> questions(UUID positionId) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new QuestionView(
                        rs.getString("question_key"), rs.getString("label"), rs.getString("type"),
                        rs.getBoolean("required"), jsonArray(rs.getString("options")))).list();
    }
    ...
    public PositionContext positionContext(UUID positionId) {
        return jdbc.sql("""
                        ...
                        """).param("id", positionId)
                .query((rs, row) -> new PositionContext(
                        rs.getObject("project_id", UUID.class), rs.getObject("hub_id", UUID.class)))
                .optional().orElseThrow(() -> new DomainException("position_not_open", "Recruitment position is not open"));
    }
    ...
    public UUID apply(UUID positionId, UUID projectId, UUID applicantId, Object answers) {
        var member = jdbc.sql("""
                        ...
                        """).param("projectId", projectId).param("accountId", applicantId)
        ...
        var formId = jdbc.sql("""
                        ...
                        """).param("id", positionId).query(UUID.class).optional()
                .orElseThrow(() -> new DomainException("recruitment_form_not_found", "Recruitment form was not found"));
        ...
        jdbc.sql("""
                        ...
                        """).param("id", id).param("positionId", positionId).param("formId", formId)
                .param("applicantId", applicantId).param("answers", json(answers)).update();
        return id;
    }
    ...
    public ApplicationContext applicationContext(UUID applicationId) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new ApplicationContext(rs.getObject("position_id", UUID.class),
                        rs.getObject("applicant_id", UUID.class), rs.getObject("project_id", UUID.class)))
                .optional().orElseThrow(() -> new DomainException("application_not_pending", "Application is not pending"));
    }

    @Override
    public void decide(UUID applicationId, ApplicationContext row, UUID reviewerId, boolean accept, String note) {
        var decided = jdbc.sql("""
                        ...
                        """).param("status", accept ? "ACCEPTED" : "REJECTED").param("reviewerId", reviewerId)
                .param("note", note).param("id", applicationId).update();
        if (decided == 0) {
        ...
        jdbc.sql("""
                        ...
                        """).param("receiver", row.applicantId()).param("actor", reviewerId)
                .param("project", row.projectId()).param("type", accept ? "APPLICATION_ACCEPTED" : "APPLICATION_REJECTED")
                .param("title", accept ? "Sua inscrição foi aprovada" : "Sua inscrição não foi aprovada")
                .param("application", applicationId).update();
    }
    ...
    public List<ApplicationView> applications(UUID projectId) {
        return jdbc.sql("""
                        ...
                        """).param("projectId", projectId)
                .query((rs, row) -> new ApplicationView(rs.getObject("id", UUID.class),
                        rs.getObject("position_id", UUID.class), rs.getString("position_title"),
                        rs.getObject("applicant_id", UUID.class), rs.getString("display_name"),
                        rs.getString("avatar_url"), rs.getString("answers"), rs.getString("status"),
                        rs.getObject("submitted_at", OffsetDateTime.class))).list();
    }

    @Override
    public UUID invite(UUID projectId, UUID accountId, UUID positionId, String functionalRole, UUID inviterId) {
        ...
        jdbc.sql("""
                        ...
                        """).param("project", projectId).param("account", accountId).update();
        var id = UUID.randomUUID();
        jdbc.sql("""
                        ...
                        """).param("id", id).param("project", projectId).param("position", positionId)
                .param("account", accountId).param("inviter", inviterId).param("role", functionalRole).update();
        jdbc.sql("""
                        insert into notifications
                            (receiver_id, actor_id, project_id, type, title, entity_type, entity_id)
                        values (:receiver, :actor, :project, 'PROJECT_INVITATION',
                                'Você recebeu um convite para projeto', 'PROJECT_INVITATION', :invitation)
                        """).param("receiver", accountId).param("actor", inviterId).param("project", projectId)
    ...
    public void respondToInvitation(UUID invitationId, UUID accountId, boolean accept) {
        var invitation = jdbc.sql("""
                        ...
                        """).param("id", invitationId).param("account", accountId)
                .query((rs, row) -> new InvitationContext(rs.getObject("project_id", UUID.class),
                        rs.getObject("position_id", UUID.class), rs.getString("functional_role")))
                .optional().orElseThrow(() -> new DomainException("invitation_not_pending", "Invitation is not pending"));
        var responded = jdbc.sql("""
                        update project_invitations set status = :status, responded_at = now()
                        where id = :id and account_id = :account and status = 'PENDING' and expires_at > now()
                        """).param("status", accept ? "ACCEPTED" : "DECLINED").param("id", invitationId)
    ...
    private void requireOptionalPosition(UUID projectId, UUID positionId) {
        ...
        var count = jdbc.sql("""
                        ...
                        """).param("position", positionId).param("project", projectId).query(Integer.class).single();
        if (count == 0) {
    ...
    private void validateRequiredAnswers(UUID formId, Object answers) {
        if (!(answers instanceof Map<?, ?> answerMap)) {
            throw new DomainException("invalid_application_answers", "Answers must be an object keyed by question");
        }
        if (answerMap.size() > 30 || json(answers).length() > 100_000) {
            throw new DomainException("application_answers_too_large", "Application answers exceed the allowed size");
        }
        var definitions = jdbc.sql("""
                        ...
                        """).param("form", formId)
                .query((rs, row) -> new AnswerDefinition(rs.getString("question_key"), rs.getString("type"),
                        rs.getBoolean("required"), jsonArray(rs.getString("options")))).list();
        if (definitions.isEmpty()) {
            var motivation = answerMap.get("motivation");
            if (!answerMap.keySet().stream().allMatch("motivation"::equals)
                    || (motivation != null && (!(motivation instanceof String) || ((String) motivation).length() > 4_000))) {
                throw new DomainException("invalid_application_answers", "Application answers do not match the form");
            }
            return;
        }
        var knownKeys = definitions.stream().map(AnswerDefinition::key).collect(java.util.stream.Collectors.toSet());
        if (!knownKeys.containsAll(answerMap.keySet())) {
            throw new DomainException("invalid_application_answers", "Application contains unknown answers");
        }
    ...
    private void validateAnswer(AnswerDefinition definition, Object value) {
        if (value == null || (value instanceof String text && text.isBlank())) {
            if (definition.required()) {
                throw new DomainException("required_answer_missing", "Required answer is missing: " + definition.key());
            }
            return;
        }
        var valid = switch (definition.type()) {
            case "BOOLEAN" -> value instanceof Boolean;
            case "SINGLE_CHOICE" -> value instanceof String text && definition.options().contains(text);
            case "MULTIPLE_CHOICE" -> value instanceof List<?> values && values.size() <= 30
                    && values.stream().allMatch(item -> item instanceof String text && definition.options().contains(text));
        ...
        if (!valid) {
            throw new DomainException("invalid_application_answer",
                    "Answer has an invalid type or value: " + definition.key());
        }
    }
    ...
    private record AnswerDefinition(String key, String type, boolean required, List<String> options) {}
}
