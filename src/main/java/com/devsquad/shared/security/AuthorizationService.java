...
public class AuthorizationService {
  ...
  public AuthorizationService(
      AccountIdentity identities, HubRoleReader hubRoles, ProjectRoleReader projectRoles) {
  ...
  public UUID requireAccount(String clerkUserId) {
    ...
    return identities
        .findActiveAccountId(clerkUserId)
        .orElseThrow(
            () ->
                new DomainException(
                    "account_not_synchronized", "Account has not been synchronized yet"));
  }

  public UUID requireHubMember(String clerkUserId, UUID hubId) {
    var accountId = requireAccount(clerkUserId);
    hubRoles
        .findActiveRole(hubId, accountId)
        .orElseThrow(
            () ->
                new DomainException(
                    "hub_membership_required", "Active hub membership is required"));
    return accountId;
  }
    ...
    var role =
        hubRoles
            ...
            .orElseThrow(
                () ->
                    new DomainException(
                        "hub_management_forbidden", "Hub management permission is required"));
  ...
  public UUID requireProjectMember(String clerkUserId, UUID projectId) {
    var accountId = requireAccount(clerkUserId);
    projectRoles
        .findActiveProjectRole(projectId, accountId)
        .orElseThrow(
            () ->
                new DomainException(
                    "project_membership_required", "Active project membership is required"));
    return accountId;
  }

  public UUID requireProjectAdmin(String clerkUserId, UUID projectId) {
    var accountId = requireAccount(clerkUserId);
    if (projectRoles.findActiveProjectRole(projectId, accountId).orElse(null)
        != com.devsquad.project.domain.ProjectRole.ADMIN) {
      throw new DomainException(
          "project_admin_required", "Project administrator permission is required");
    }
    return accountId;
  }
