...
public class AttachmentService {
  ...
  public AttachmentService(
      AttachmentStore attachments, AttachmentStorage storage, AuthorizationService authorization) {
  ...
  public UploadTicket requestUpload(String clerkId, UploadCommand command) {
    ...
    if (!attachments.parentBelongsToProject(command)) {
      throw new DomainException(
          "invalid_attachment_parent", "Attachment parent is not in this project");
    }
  ...
  private void requireValid(UploadCommand command) {
    ...
    if (command.sizeBytes() <= 0 || command.sizeBytes() > storage.maxFileSize()) {
      throw new DomainException(
          "invalid_attachment_size", "Attachment size is outside the allowed range");
    }
  }
  ...
  public record UploadTicket(
      UUID attachmentId, String uploadUrl, Map<String, java.util.List<String>> headers) {}
