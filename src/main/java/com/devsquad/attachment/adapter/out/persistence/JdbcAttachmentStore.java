...
public class JdbcAttachmentStore implements AttachmentStore {
    ...
    public boolean parentBelongsToProject(UploadCommand command) {
        var count = command.taskId() != null
                ? jdbc.sql("select count(*) from tasks where id = :id and project_id = :project")
                    .param("id", command.taskId()).param("project", command.projectId()).query(Integer.class).single()
                : jdbc.sql("""
                            ...
                            """).param("id", command.commentId()).param("project", command.projectId())
    ...
    public void create(UUID id, UUID actorId, String objectKey, UploadCommand command) {
        jdbc.sql("""
                        ...
                        """)
                .param("id", id).param("project", command.projectId()).param("task", command.taskId())
                .param("comment", command.commentId()).param("actor", actorId).param("key", objectKey)
                .param("name", command.fileName()).param("type", command.contentType()).param("size", command.sizeBytes())
                .update();
    }
    ...
    public List<AttachmentView> findByTask(UUID taskId) {
        return jdbc.sql("""
                        ...
                        """).param("task", taskId)
                .query((rs, row) -> new AttachmentView(rs.getObject("id", UUID.class),
    ...
    private StoredAttachment find(UUID id, String status, String code, String message) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new StoredAttachment(
                        rs.getObject("project_id", UUID.class), rs.getString("object_key"), rs.getLong("size_bytes")))
                .optional().orElseThrow(() -> new DomainException(code, message));
    }
}
