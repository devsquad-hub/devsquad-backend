...
public class HubMembershipService {
  ...
  public List<HubMembershipStore.MemberView> members(String clerkUserId, UUID hubId) {
    ...
    return memberships.findMembers(hubId).stream()
        .map(
            member ->
                new HubMembershipStore.MemberView(
                    member.accountId(),
                    member.displayName(),
                    showEmail ? member.email() : null,
                    member.avatarUrl(),
                    member.role()))
        .toList();
  }
  ...
  public void assign(String clerkUserId, UUID hubId, UUID accountId, HubRole role) {
    ...
    if (actorId.equals(accountId)) {
      throw new DomainException(
          "master_self_demotion_forbidden", "The hub master cannot change their own role");
    }
    if (role == HubRole.MASTER) {
      throw new DomainException(
          "master_transfer_not_supported", "Use the explicit master transfer operation");
    }
