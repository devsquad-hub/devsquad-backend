...
public class JdbcProjectStore implements ProjectStore {
    ...
    public JdbcProjectStore(JdbcClient jdbc) { this.jdbc = jdbc; }

    @Override
    public Optional<ProjectView> find(UUID projectId) {
        return jdbc.sql(SELECT + " where p.id = :id" + GROUP).param("id", projectId)
                .query((rs, row) -> map(rs)).optional();
    }

    @Override
    public List<ProjectView> findPublic(UUID hubId) {
        return jdbc.sql(SELECT + " where p.hub_id = :hubId and p.status <> 'ARCHIVED'" + GROUP + "order by p.created_at desc")
                .param("hubId", hubId).query((rs, row) -> map(rs)).list();
    }

    @Override
    public Optional<ProjectView> findPublicBySlug(String hubSlug, String projectSlug) {
        return jdbc.sql(SELECT + " join hubs h on h.id = p.hub_id where h.slug = :hubSlug and p.slug = :projectSlug" + GROUP)
                .param("hubSlug", hubSlug).param("projectSlug", projectSlug).query((rs, row) -> map(rs)).optional();
    }

    @Override
    public ProjectView updateStatus(UUID projectId, ProjectStatus expectedStatus, ProjectStatus status) {
        var updated = jdbc.sql("""
                        update projects set status = :status, updated_at = now(), version = version + 1
                        where id = :id and status = :expectedStatus
                        """).param("status", status.name()).param("expectedStatus", expectedStatus.name())
    ...
    public ProjectView update(UUID projectId, ProjectUpdate update) {
        jdbc.sql("""
                        ...
                        where id = :id
                        """).param("name", update.name().trim()).param("summary", update.summary().trim())
                .param("description", update.description()).param("repositoryUrl", update.repositoryUrl())
                .param("communicationUrl", update.communicationUrl()).param("tags", pgArray(update.tags()))
                .param("id", projectId).update();
        return find(projectId).orElseThrow();
    }
    ...
    public void assignAdmin(UUID projectId, UUID accountId) {
        jdbc.sql("""
                        ...
                        """)
                .param("projectId", projectId).param("accountId", accountId).update();
    }
    ...
    public boolean isActiveHubMember(UUID hubId, UUID accountId) {
        return jdbc.sql("""
                        ...
                        """).param("hubId", hubId).param("accountId", accountId).query(Integer.class).single() > 0;
    }
    ...
    private ProjectView map(ResultSet rs) throws SQLException {
        ...
        return new ProjectView(id, rs.getObject("hub_id", UUID.class), rs.getString("name"), rs.getString("slug"),
                rs.getString("project_key"), rs.getString("summary"), rs.getString("description"),
                ProjectStatus.valueOf(rs.getString("status")), rs.getString("repository_url"), rs.getString("communication_url"),
                List.copyOf(tags), rs.getLong("total_tasks"), rs.getLong("completed_tasks"), members(id));
    }
    ...
    private List<MemberView> members(UUID projectId) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new MemberView(rs.getObject("id", UUID.class), rs.getString("display_name"),
                        rs.getString("avatar_url"), rs.getString("role"), rs.getString("functional_role"))).list();
    }
