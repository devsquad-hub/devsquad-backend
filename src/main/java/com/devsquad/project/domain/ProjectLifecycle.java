...
public final class ProjectLifecycle {
  ...
  public void transitionTo(ProjectStatus next) {
    if (!TRANSITIONS.get(status).contains(next)) {
      throw new DomainException(
          "invalid_project_transition", "Cannot transition project from " + status + " to " + next);
    }
    status = next;
  }
  ...
  private static Map<ProjectStatus, EnumSet<ProjectStatus>> transitions() {
    ...
    transitions.put(
        ProjectStatus.PLANNING, EnumSet.of(ProjectStatus.RECRUITING, ProjectStatus.ARCHIVED));
    transitions.put(
        ProjectStatus.RECRUITING, EnumSet.of(ProjectStatus.ACTIVE, ProjectStatus.ARCHIVED));
    transitions.put(
        ProjectStatus.ACTIVE, EnumSet.of(ProjectStatus.COMPLETED, ProjectStatus.ARCHIVED));
