...
public class ProjectService {
    ...
    public ProjectStore.ProjectView publicProject(String hubSlug, String projectSlug) {
        return projects.findPublicBySlug(hubSlug, projectSlug)
    ...
    public ProjectDetails internal(String clerkId, UUID projectId) {
        ...
        return new ProjectDetails(project, authorization.capabilities(clerkId, project.hubId(), projectId));
    }
    ...
    public void assignAdmin(String clerkId, UUID projectId, UUID accountId) {
        ...
        if (!projects.isActiveHubMember(project.hubId(), accountId)) {
            throw new DomainException("project_admin_must_be_hub_member", "Project administrator must be an active hub member");
        }
        projects.assignAdmin(projectId, accountId);
    }

    private ProjectStore.ProjectView get(UUID projectId) {
        return projects.find(projectId).orElseThrow(() -> new DomainException("project_not_found", "Project was not found"));
    }

    public record ProjectDetails(ProjectStore.ProjectView project, ViewerCapabilities viewerCapabilities) {}
}
