...
public class JdbcAccountStore implements AccountStore {
    ...
    private static Account map(ResultSet rs, int rowNumber) throws SQLException {
        ...
        return new Account(
                rs.getObject("id", java.util.UUID.class), rs.getString("clerk_user_id"), rs.getString("email"),
                rs.getString("display_name"), rs.getString("avatar_url"), rs.getString("bio"), List.copyOf(skills),
