...
public class JdbcPersonalWorkspaceStore implements PersonalWorkspaceStore {
    ...
    public List<ProjectStore.ProjectView> projects(UUID accountId) {
        return jdbc.sql("""
                        ...
                        """).param("accountId", accountId).query(UUID.class).list().stream()
                .map(projects::find).flatMap(java.util.Optional::stream).toList();
    }
    ...
    public List<ApplicationSummary> applications(UUID accountId) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new ApplicationSummary(
                        rs.getObject("id", UUID.class), rs.getObject("project_id", UUID.class),
                        rs.getString("project_name"), rs.getObject("position_id", UUID.class),
                        rs.getString("position_title"), rs.getString("status"),
    ...
    public List<InvitationSummary> invitations(UUID accountId) {
        return jdbc.sql("""
                        ...
                        """).param("accountId", accountId)
                .query((rs, row) -> new InvitationSummary(
                        rs.getObject("id", UUID.class), rs.getObject("project_id", UUID.class),
                        rs.getString("project_name"), rs.getObject("position_id", UUID.class),
                        rs.getString("functional_role"), rs.getString("status"),
                        rs.getObject("expires_at", OffsetDateTime.class))).list();
    }
}
