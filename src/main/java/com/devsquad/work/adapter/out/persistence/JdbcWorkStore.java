...
public class JdbcWorkStore implements WorkStore {
  ...
  public TaskView createTask(UUID projectId, UUID actorId, TaskCommand command) {
    ...
    jdbc.sql(
        ...
        .param("parentId", command.parentId())
        .param("columnId", command.columnId())
        .param("milestoneId", command.milestoneId())
        .param("title", command.title().trim())
        .param("description", command.description())
        .param("priority", command.priority())
        .param("startDate", command.startDate())
  ...
  public TaskView updateTask(
      UUID taskId, UUID projectId, UUID actorId, TaskCommand command, int expectedVersion) {
    requireColumn(projectId, command.columnId());
    requireOptionalParent(projectId, command.parentId(), "tasks", "parent_task_not_found");
    requireOptionalParent(projectId, command.milestoneId(), "milestones", "milestone_not_found");
    var updated =
  ...
  public TaskView move(
      UUID taskId, UUID projectId, UUID actorId, UUID columnId, int position, int expectedVersion) {
    ...
    var updated =
        jdbc.sql(
            ...
            .param("completed", "COMPLETED".equals(group))
            .param("id", taskId)
            .param("version", expectedVersion)
            .update();
    if (updated == 0) {
  ...
  public UUID comment(UUID taskId, UUID projectId, UUID actorId, String body) {
    var id = UUID.randomUUID();
    jdbc.sql(
            "insert into comments (id, task_id, author_id, body) values (:id, :taskId, :authorId,"
                + " :body)")
        .param("id", id)
        .param("taskId", taskId)
        .param("authorId", actorId)
        .param("body", body)
        .update();
  ...
  public UUID createLabel(UUID projectId, String name, String color) {
    var id = UUID.randomUUID();
    jdbc.sql(
            "insert into labels (id, project_id, name, color) values (:id, :project, :name,"
                + " :color)")
        .param("id", id)
        .param("project", projectId)
        .param("name", name)
  ...
  public void addLabel(UUID taskId, UUID projectId, UUID labelId) {
    var matching =
        jdbc.sql("select count(*) from labels where id = :label and project_id = :project")
            .param("label", labelId)
            .param("project", projectId)
            .query(Integer.class)
            .single();
    ...
    jdbc.sql(
            "insert into task_labels (task_id, label_id) values (:task, :label) on conflict do"
                + " nothing")
        .param("task", taskId)
        .param("label", labelId)
        .update();
  ...
  public void addAssignee(UUID taskId, UUID projectId, UUID accountId) {
    ...
    jdbc.sql(
            "insert into task_assignees (task_id, account_id) values (:task, :account) on conflict"
                + " do nothing")
        .param("task", taskId)
        ...
        .query(
            (rs, row) ->
                new TaskView(
                    ...
                    assignees(rs.getObject("id", UUID.class))))
        .list();
  }
        ...
        .query(
            (rs, row) ->
                new TaskView(
                    ...
                    rs.getLong("sequence"),
                    rs.getObject("parent_id", UUID.class),
                    rs.getObject("column_id", UUID.class),
                    rs.getObject("milestone_id", UUID.class),
                    rs.getString("title"),
                    rs.getString("description"),
                    rs.getString("priority"),
  ...
  private void replaceAssignees(UUID taskId, UUID projectId, List<UUID> assigneeIds) {
    ...
    for (var accountId : assigneeIds) {
      requireProjectMember(projectId, accountId);
      jdbc.sql(
              "insert into task_assignees (task_id, account_id) values (:task, :account) on"
                  + " conflict do nothing")
          .param("task", taskId)
          .param("account", accountId)
          .update();
  ...
  private void requireProjectMember(UUID projectId, UUID accountId) {
    ...
    if (active == 0) {
      throw new DomainException(
          "assignee_not_project_member", "Every assignee must be a project member");
    }
  }
  ...
  private String requireColumn(UUID projectId, UUID columnId) {
    return jdbc.sql(
        ...
        .orElseThrow(
            () ->
                new DomainException("workflow_column_not_found", "Workflow column was not found"));
  }
  ...
  private void recordActivity(
      UUID projectId, UUID actorId, String event, String entity, UUID entityId) {
    jdbc.sql(
            """
