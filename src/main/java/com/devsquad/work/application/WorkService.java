...
public class WorkService {
    ...
    @Transactional
    public TaskView updateTask(String clerkId, UUID taskId, TaskCommand command, int expectedVersion) {
        requirePriority(command.priority());
    ...
    @Transactional
    public TaskView move(String clerkId, UUID taskId, UUID columnId, int position, int expectedVersion) {
        var projectId = work.projectForTask(taskId);
        var actorId = authorization.requireProjectMember(clerkId, projectId);
    ...
    @Transactional
    public UUID createColumn(String clerkId, UUID projectId, String name, String semanticGroup, int position) {
    ...
    public record TaskCommand(UUID parentId, UUID columnId, UUID milestoneId, String title, String description,
                              String priority, LocalDate startDate, LocalDate dueDate, int position, List<UUID> assigneeIds) {}
    public record MilestoneCommand(String title, String description, LocalDate startDate, LocalDate dueDate) {}
    public record BoardView(UUID projectId, List<ColumnView> columns) {}
    public record ColumnView(UUID id, String name, String semanticGroup, int position, List<TaskView> tasks) {}
    public record TaskView(UUID id, long sequence, UUID parentId, UUID columnId, UUID milestoneId, String title,
                           String description, String priority, LocalDate startDate, LocalDate dueDate, int position,
                           int version, OffsetDateTime completedAt, List<AssigneeView> assignees) {}
    public record AssigneeView(UUID id, String displayName, String avatarUrl) {}
    public record CommentView(UUID id, String body, OffsetDateTime createdAt, UUID authorId, String authorName,
                              String authorAvatarUrl) {}
    public record ActivityView(UUID id, String eventType, String entityType, UUID entityId, String actorName,
